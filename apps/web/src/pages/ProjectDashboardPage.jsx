    ...
    <div className="space-y-6">
          ...
          <div className="flex justify-between items-start z-10 relative">
            <div>
              ...
              <h3 className="font-headline-lg text-headline-lg text-primary mt-1">
                {(stats.totalStorageBytes / 1024 / 1024).toFixed(2)}{" "}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono-code text-mono-code text-on-surface-variant group-hover:text-primary transition-colors">
            Search commands, navigate workspace, or find documents…
          </span>
        </div>
        <kbd className="px-2 py-0.5 level-2 rounded border border-outline-variant font-mono-label text-[10px] text-on-surface-variant tracking-wider">
          CTRL + K
        </kbd>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 level-1 rounded-DEFAULT p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link
              ...
              className="font-mono-label text-mono-label text-on-surface-variant hover:text-primary transition-colors flex items-center gap-1"
            >
              View All{" "}
          ...
          <div className="space-y-3 flex-1">
            {stats.latestDocuments.length ? (
              stats.latestDocuments.map((doc) => (
                <div
                  key={doc._id}
                  className="level-2 p-4 rounded-DEFAULT flex items-center justify-between group hover:border-outline-variant transition-colors"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-body-md text-body-md font-bold text-primary">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="hidden sm:block text-right">
                      ...
                      <p className="font-mono-code text-mono-code text-primary capitalize">
                        {doc.status}
                      </p>
                    </div>
                    <div className="w-24 h-1.5 bg-surface-container-high rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${doc.status === "ready" ? "bg-primary w-full" : "bg-outline-variant w-1/2"}`}
                      ></div>
                    </div>
                  </div>
                </div>
              ...
              <p className="text-sm font-mono-code text-on-surface-variant">
                No documents isolated in current space.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="level-1 rounded-DEFAULT p-6 flex flex-col">
          ...
          <Link
            ...
            className="mt-auto pt-4 w-full block text-center font-mono-label text-mono-label text-on-surface-variant hover:text-primary transition-colors border-t border-outline-variant/30"
          >
            Open Chat Core
          </Link>
        </div>
